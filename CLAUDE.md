...
Dépôt GitHub : git@github.com:ethersys/ethersys-importer-for-modelo-netty.git

Plugin WordPress mono-paquet (PHP 8.3+). Le code source distribuable est intégralement dans `ethersys-importer-for-modelo-netty/` ; le dossier doit être copié tel quel dans `wp-content/plugins/`. Pas de build, pas de bundler, pas de gestionnaire de dépendances (PHP ou JS) — tout est en PHP natif + un script inline pour le front DPE/GES.
...
Pour validation manuelle : installer le plugin dans un WordPress local + thème Houzez, configurer l’URL de flux dans **Admin → Import Netty**, déclencher l’import et lire les logs (table `{prefix}eimn_import_logs`, écran admin du plugin).

### Packager pour distribution

```bash
# Le .gitignore exclut déjà les zips. Le zip doit contenir le dossier "ethersys-importer-for-modelo-netty/"
# à la racine pour que WordPress le reconnaisse.
(cd ethersys-importer-for-modelo-netty && zip -r ../ethersys-importer-for-modelo-netty.zip .)
```

### Traductions

Fichiers dans `ethersys-importer-for-modelo-netty/languages/` : `netty-houzez-importer.pot` (template) et `netty-houzez-importer-fr_FR.po`. Le text domain réel utilisé en code est `ethersys-importer-for-modelo-netty` (constante `Plugin::TEXT_DOMAIN`). Régénérer avec `wp i18n make-pot netty-to-wp-import ethersys-importer-for-modelo-netty/languages/netty-houzez-importer.pot --domain=ethersys-importer-for-modelo-netty`.

## Architecture

### Bootstrap

`ethersys-importer-for-modelo-netty.php` définit les constantes `EIMN_VERSION`, `EIMN_PATH`, `EIMN_URL`, `EIMN_BASENAME` puis délègue à `Plugin::init()` (dans `includes/class-plugin.php`). `Plugin::init()` charge tous les `class-*.php` du dossier `includes/` puis appelle `::init()` sur chaque sous-système. Les hooks d’activation/désactivation y sont aussi enregistrés (création de tables + (re)programmation cron).

Tout le code vit dans le namespace `NettyWP\Import`. Préfixe pour les options, hooks, transients, tables : `eimn_`.
...
- `Cron` enregistre une recurrence dynamique `eimn_import_recurrence` dont l’intervalle est recalculé depuis `eimn_schedule_interval` × `eimn_schedule_unit` (capé à 30 jours). À chaque enregistrement des réglages, `Admin::handle_save_settings()` appelle `Cron::reschedule_main_import()` qui clear + reschedule. Si l’URL du flux est invalide, **aucun import auto n’est planifié**.
- Une seconde recurrence `eimn_purge_daily` purge les runs anciens (`Db::purge_old_runs()`, garde 200 runs).
- Le verrou d’import est une **option** (`eimn_import_lock`) posée atomiquement via `add_option()` (`Importer::run()`) pour empêcher deux imports simultanés — pas un transient. Elle est libérée dans le `finally` de `run()` et, en cas de run resté bloqué, nettoyée par `cleanup_stale_runs()` après `STALE_RUN_MINS` (30 min).
...
- `{prefix}eimn_import_runs` : un row par exécution (status, dates, source_url, counts_json, error_message).
- `{prefix}eimn_import_logs` : lignes de log liées à un run, indexées par `run_id`, `level`, `action`, `reference_technique`.
...
- Court-circuiter / remplacer le téléchargement d’images → filtre `eimn_pre_download_urls` (`$pre, array $urls, int $timeout`) en tête de `MediaSync::download_urls_parallel()`. Retourner un tableau `map url => chemin tmp | WP_Error` court-circuite `curl_multi` (le reste du pipeline — sideload, traçage `eimn_source_url`, suppression — est inchangé) ; retourner `null` laisse le téléchargement normal s’exécuter. Sert notamment aux tests d’intégration (`MediaSyncIntegrationTest`) pour injecter des fichiers fixtures sans réseau.
